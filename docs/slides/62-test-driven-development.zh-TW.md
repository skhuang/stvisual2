...
1. 從 kata 選擇器選取 **FizzBuzz** kata。
   - 測試清單面板顯示初始失敗測試（`fizzbuzz(1) === "1"`）。
   - 階段指示器顯示**紅燈**。
2. 點擊**下一步** ── 程式碼面板填入 `function fizzbuzz(n) { return "1"; }`。
   - 階段變為**綠燈**；套件計數器顯示 1 通過，0 失敗。
3. 繼續逐步執行 s3–s9 ── 觀察階段轉換：紅燈→綠燈→紅燈→綠燈→紅燈→綠燈→紅燈→綠燈→重構。
4. 在每個紅燈步驟，**預測**哪個測試會失敗，然後再點擊下一步——對照套件計數器驗證你的預測。
5. 切換到 **Stack** kata 並重複——注意 isEmpty、push、pop 是如何一次一個行為地引入的。
...
1. 探索器顯示三條 TDD 規則和一個模擬的程式碼編輯器。
2. 嘗試在測試套件為綠燈且不存在失敗測試時**寫生產程式碼**。
   - 工具阻止該動作並高亮顯示**規則 1**：「沒有失敗測試，不寫任何生產程式碼。」
3. 寫一個失敗測試，然後嘗試**一次寫兩個測試**（在第一個通過之前新增第二個 `it()` 區塊）。
   - 工具阻止並高亮顯示**規則 2**：「只寫剛好讓一個測試失敗的測試程式碼。」
4. 有一個失敗測試後，嘗試**寫超過需要的程式碼**——第二個函式、一個失敗測試未要求的額外分支。
   - 工具阻止並高亮顯示**規則 3**：「只寫剛好讓失敗測試通過的生產程式碼。」
5. 遵守全部三條規則完成一個完整的紅燈－綠燈－重構循環——觀察套件全程保持乾淨。
